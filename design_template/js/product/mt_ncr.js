...
var setAmt = function() {
    ...
    modulo = 0;
    if(stan_mpcode == '813' || stan_mpcode == '803' || stan_mpcode == '793') { // A3, 5의 배수
        modulo = 5;
    } else if(stan_mpcode == '814' || stan_mpcode == '804' || stan_mpcode == '794') { // A4, 10의 배수
        modulo = 10;
    } else if(stan_mpcode == '815' || stan_mpcode == '805' || stan_mpcode == '795') { // A4 1/3 30의 배수
        modulo = 30;
    } else if(stan_mpcode == '816' || stan_mpcode == '806' || stan_mpcode == '796') { // A5 20의 배수
        modulo = 20;
    } else if(stan_mpcode == '817' || stan_mpcode == '807' || stan_mpcode == '797') { // A6 40의 배수
        modulo = 40;
    } else if(stan_mpcode == '818' || stan_mpcode == '808' || stan_mpcode == '798') { // 8절 5의 배수
        modulo = 5;
    } else if(stan_mpcode == '819' || stan_mpcode == '809' || stan_mpcode == '799') { // 16절, 10의 배수
        modulo = 10;
    } else if(stan_mpcode == '820' || stan_mpcode == '810' || stan_mpcode == '800') { // 32절 20의 배수
        modulo = 20;
    } else if(stan_mpcode == '821' || stan_mpcode == '811' || stan_mpcode == '801') { // 48절 30의 배수
        modulo = 30;
    } else if(stan_mpcode == '822' || stan_mpcode == '812' || stan_mpcode == '802') { // 64절 40의 배수
        modulo = 40;
    }
...
var getPaperDetail = function() {
    ...
    if($("#bef_aft_diff_yn").is("checked")) {
        str_diff = "Y";
    }
    str += "다름여부 : " + str_diff + " / ";

    if($("#sortcode").find("option:selected").val() == "007001001") {
        str_color += $("#paperTop").find("option:selected").val()  + ",";
        str_color += $("#paperBot").find("option:selected").val();
    } else if($("#sortcode").find("option:selected").val() == "007001002") {
        str_color += $("#paperTop").find("option:selected").val() + ",";
        str_color += $("#paperMid1").find("option:selected").val() + ",";
        str_color += $("#paperBot").find("option:selected").val();
    } else if($("#sortcode").find("option:selected").val() == "007001003") {
        str_color += $("#paperTop").find("option:selected").val() + ",";
        str_color += $("#paperMid1").find("option:selected").val() + ",";
        str_color += $("#paperMid2").find("option:selected").val() + ",";
        str_color += $("#paperBot").find("option:selected").val();
    }
