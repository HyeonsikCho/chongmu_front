...
$(document).ready(function() {
    ...
    $("input[name='rounding_dvs']").change(function () {
        i = 0;
        cnt_value = $("#rounding option:selected").val();
        limit = 0;
        if(cnt_value == "한귀도리") {
            limit = 1;
        } else if(cnt_value == "두귀도리") {
            limit = 2;
        } else if(cnt_value == "세귀도리") {
            limit = 3;
        } else if(cnt_value == "네귀도리") {
            limit = 4;
        }

        $("input[name='rounding_dvs']").each(function() {
            if($(this).is(":checked")) {
                i++;
            }
        });
        ...
        if(limit < i) {
            alert(cnt_value + "에는 " + limit + "개의 귀도리까지만 들어 갈 수 있습니다.");
	...
	$("._productName option[value='001001003']").hide();
    orderSummary();
    changePaper();
});
...
var goCart = function(flag) {
    ...
    //var manu_pos_num = $("#manu_pos_num").val();
    ...
    //$("#frm").find("input[name='manu_pos_num']").val(manu_pos_num);
    $("#manu_pos_num").attr("disabled", false);
//	console.log($('#frm').formSerialize());
	...
	var callback = function(data) {
		if(flag == 'N'){
			if(data.result == 'true'){
                $("#manu_pos_num").attr("disabled", true);
...
var changePaper = function() {
    ...
    $("#bef_tmpt_cover > option").each(function() {
        $(this).show();
    });
    ...
    } else if(selectedValue == '244' || selectedValue == '247' || selectedValue == '248' || selectedValue == '252' || selectedValue == '242' || selectedValue == '254') {
        $("#bef_tmpt_cover option:eq(1)").hide();
    }
