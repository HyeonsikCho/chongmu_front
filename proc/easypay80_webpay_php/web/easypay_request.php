...
/* 배열값을 일괄적으로 UTF-8로 변환 */
function utf8_encode_arr(&$item){
...
array_walk($easyPay->_easypay_resdata,'utf8_encode_arr');
...
/* -------------------------------------------------------------------------- */
if ( $res_cd == "0000" && $pay_type != '22') { //가상계좌 일때는 결제완료 테이블에 들어가지않음
    $bDBProc = 'false';
    $param['ohash'] = $fb->ss['ohash'];
    $param['user_id'] = $fb->ss['MYSEC_ID'];

    foreach($easyPay->_easypay_resdata as $key=>$value){
        $param[$key] = $value;
    }
    $param['plzp_id'] = $fb->fb['EP_user_define1'];
    $param['plzp_point'] = $fb->fb['EP_user_define2'];
    $param['prkVal'] = $fb->fb['EP_user_define3'];
    $param['cp_pay_type'] = 99;
    // 총 결제금액은 포인트 + 실결제금액
    $param['t_amount'] = (int)$param['amount'] + (int)$param['plzp_point'];
    ...
    if($o_res){
        ...
        while($prs && !$prs->EOF){
            ...
            $param['used_mysec'] = $prs->fields['used_mileage'];
            $acc_point += $prs->fields['used_mileage'];
            ...
            if($i == $p_cnt){
                if($param['plzp_point'] > $acc_point){
                    $param['used_mysec'] += $param['plzp_point'] - $acc_point;
                }else if($param['plzp_point'] < $acc_point){
                    $param['used_mysec'] -= $param['plzp_point'] - $acc_point;
                }
            }
            $i++;
...
<script type="text/javascript">
    function f_submit() {
        ...
        if(pay_type == '11') { // 카드결제시(결제완료)
            frm.action = '/order/complete.html';
            frm.submit();
        }
        else if(pay_type == '22') {// 가상계좌 이용시(결제정보 보여주기)
            frm.action = '/order/requestdeposit.html';
            frm.submit();
        }
    }
</script>
...
<form name="frm" method="post">
    ...
    <input type='hidden' id='t_amount'         name='t_amount'        value='<?=$param['t_amount']?>'>
    ...
    <input type="hidden" id="plzp_point"        name="plzp_point"       value="<?echo (int)$param['plzp_point'];?>">
    <input type="hidden" id="EP_product_nm"        name="EP_product_nm"       value="<?echo $param['EP_product_nm'];?>">
